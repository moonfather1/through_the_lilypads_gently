...
public class TaskScheduler
{
    ...
    public static void onLevelTick(LevelTickEvent.Post tickEvent)
    {
        if (tickEvent.getLevel().isClientSide()) { return; }
        Iterator<ScheduledEvent> i = list.iterator();
        while (i.hasNext())
        {
            ScheduledEvent e = i.next();
            if (! e.level.equals(tickEvent.getLevel())) { continue; }
            e.tickCount -= 1;
            if (e.tickCount <= 0)
            {
        ...
        while (i2.hasNext())
        {
            ...
            if (! tickEvent.getLevel().equals(e.level)) { continue; }
            e.currentTick += 1;
